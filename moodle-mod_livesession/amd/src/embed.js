...
/**
 * The size the meeting canvas should be created at, taken from the activity frame.
 *
 * @param {HTMLElement} frame the fluid element the meeting sits in
 * @returns {{width: Number, height: Number}}
 */
const frameSize = (frame) => {
    const width = Math.max(320, Math.round(frame.getBoundingClientRect().width || 960));
    // 16:9, but never so tall that Zoom's toolbar falls below the fold on a laptop.
    const height = Math.min(
        Math.round(width * 9 / 16),
        Math.max(360, Math.round(window.innerHeight * 0.78))
    );
    return {width, height};
};

/**
 * Scale the stage so the meeting always fits the frame's current width.
 *
 * Zoom fixes the canvas size when the client initialises and offers no way to change
 * it afterwards, so a window that narrows mid-meeting would otherwise clip the
 * meeting rather than reflow it. Scaling the stage keeps the whole canvas visible.
 * It only ever scales down: enlarging a fixed-size canvas just blurs it, so a wider
 * window leaves the meeting at the size it was created and the extra space unused
 * until the page is reloaded.
 *
 * @param {HTMLElement} frame
 * @param {HTMLElement} stage
 * @param {{width: Number, height: Number}} base the size the meeting was created at
 * @returns {void}
 */
const fitStageToFrame = (frame, stage, base) => {
    if (!frame || !stage || !base.width) {
        return;
    }
    ...
    const scale = Math.min(1, Math.max(0.35, available / base.width));
    stage.style.width = `${base.width}px`;
    stage.style.height = `${base.height}px`;
    stage.style.transform = scale === 1 ? '' : `scale(${scale})`;
    // The frame must shrink with the scaled stage or it leaves dead space below.
    frame.style.height = `${Math.round(base.height * scale)}px`;
};

/**
 * Keep the meeting fitted as the window or the surrounding layout changes.
 *
 * @param {HTMLElement} frame
 * @param {HTMLElement} stage
 ...
 * @returns {void}
 */
const watchFrameSize = (frame, stage, base) => {
    let pending = null;
    const refit = () => {
        window.clearTimeout(pending);
        pending = window.setTimeout(() => fitStageToFrame(frame, stage, base), 100);
    };

    window.addEventListener('resize', refit);
    window.addEventListener('orientationchange', refit);

    // Catches the frame changing width without the window doing so - a block drawer
    // opening, or the theme switching to a narrower layout.
    if (typeof ResizeObserver === 'function') {
        new ResizeObserver(refit).observe(frame);
    }

    fitStageToFrame(frame, stage, base);
};
 ...
 * @returns {Promise<void>}
 */
const startMeeting = async(root, statusEl, counterEl, joinButton, frame, stage) => {
    ...
    // Created at the activity frame's width so the meeting fills the page rather
    // than sitting in a fixed box, then held to that size - Zoom cannot change it
    // afterwards, and letting the user drag it would break the fit.
    const size = frameSize(frame || root);
    if (stage) {
    ...
    // defaultViewType is honoured at init, but a meeting that remembers a previous
    // layout can come back in gallery view; asking again once we are in settles it.
    if (typeof zoomClient.setViewType === 'function') {
        try {
            await zoomClient.setViewType('speaker');
        } catch (error) {
            Log.debug('mod_livesession: could not force speaker view.', error);
        }
    }

    if (frame && stage) {
        frame.classList.remove('mod-livesession-idle');
        watchFrameSize(frame, stage, size);
    }
 ...
 * @param {String} frameId id of the fluid frame the meeting is fitted into
 * @param {String} stageId id of the fixed-size stage that is scaled to fit
 * @returns {void}
 */
export const init = (cmid, rootId, buttonId, statusId, counterId, frameId, stageId) => {
    ...
    if (frame) {
        frame.classList.add('mod-livesession-idle');
    }
    ...
    joinButton.addEventListener('click', async(e) => {
        ...
        try {
            await startMeeting(root, statusEl, counterEl, joinButton, frame, stage);
        } catch (error) {
